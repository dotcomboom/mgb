package mx.managers
{
   ...
   public class SystemManager extends MovieClip implements IChildList, IFlexDisplayObject, IFlexModuleFactory, ISystemManager, ISWFBridgeProvider
   {
      ...
      mx_internal static function registerInitCallback(param1:Function) : void
      {
         ...
         var _loc2_:SystemManager = mx_internal::lastSystemManager;
      ...
      private function findLastActiveForm(param1:Object) : Object
      {
         ...
         var _loc3_:int = int(forms.length - 1);
         while(_loc3_ >= 0)
         {
      ...
      private function mouseDownHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(_loc2_) && mx_internal::bridgeToFocusManager[_loc2_] == document.focusManager)
         {
            if(isTopLevelRoot())
            {
               activateForm(document);
               ...
               while(_loc4_)
               {
                  ...
                  while(_loc6_ < _loc3_)
                  {
                     if((_loc7_ = isRemotePopUp(forms[_loc6_]) ? forms[_loc6_].window : forms[_loc6_]) == _loc4_)
                     {
                        ...
                        _loc10_ = _loc9_ = _loc11_.getChildIndex(_loc4_);
                        _loc3_ = int(forms.length);
                        _loc8_ = 0;
      ...
      private function removePopupRequestHandler(param1:Event) : void
      {
         ...
         var _loc2_:SWFBridgeRequest = SWFBridgeRequest.marshal(param1);
         if(Boolean(swfBridgeGroup.parentBridge) && SecurityUtil.hasMutualTrustBetweenParentAndChild(this))
         {
      ...
      private function findRemotePopUp(param1:Object, param2:IEventDispatcher) : RemotePopUp
      {
         ...
         var _loc3_:int = int(forms.length);
         var _loc4_:int = 0;
         while(_loc4_ < _loc3_)
         {
      ...
      {
         var _loc1_:int = int(forms.length);
         if(_loc1_ < 2)
         {
            return;
         }
      ...
      private function initializeTopLevelWindow(param1:Event) : void
      {
         ...
         if(document)
         {
            ...
            if(!LoaderConfig.mx_internal::_url)
            {
