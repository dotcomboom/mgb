package mx.skins.halo
{
   ...
   public class TabSkin extends Border
   {
      ...
      private static function isTabNavigator(param1:Object) : Boolean
      {
         ...
         xmllist = x.extendsClass.(@type == "mx.containers::TabNavigator");
         if(xmllist.length() == 0)
         {
            tabnavs[s] = 0;
            return false;
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         if(_loc13_)
         {
            _loc14_ = Number(Object(parent.parent.parent).borderMetrics.top);
         }
            ...
            case "selectedDisabledSkin":
               if(isNaN(_loc4_))
               {
                  ...
                  while(_loc25_)
                  {
                     ...
                     {
                        _loc4_ = Number(IStyleClient(_loc25_).getStyle("backgroundColor"));
                     }
                     if(!isNaN(_loc4_))
                     {
                        break;
