package mx.controls.dataGridClasses
{
   ...
   public class DataGridDragProxy extends UIComponent
   {
      ...
      override protected function createChildren() : void
      {
         ...
         while(_loc3_ < _loc2_)
         {
            if(_loc4_ = DataGridBase(owner).itemToItemRenderer(_loc1_[_loc3_]))
            {
               _loc6_ = _loc1_[_loc3_];
               _loc5_ = new UIComponent();
               addChild(DisplayObject(_loc5_));
               _loc7_ = 0;
               if(DataGridBase(owner).mx_internal::visibleLockedColumns)
               {
                  _loc8_ = int(DataGridBase(owner).mx_internal::visibleLockedColumns.length);
                  _loc9_ = 0;
                  while(_loc9_ < _loc8_)
                  {
                     _loc10_ = DataGridBase(owner).mx_internal::visibleLockedColumns[_loc9_];
                     _loc11_ = DataGridBase(owner).createColumnItemRenderer(_loc10_,false,_loc6_);
                     _loc12_ = new DataGridListData(_loc10_.itemToLabel(_loc6_),_loc10_.dataField,_loc10_.mx_internal::colNum,"",DataGridBase(owner));
               ...
               _loc8_ = int(DataGridBase(owner).mx_internal::visibleColumns.length);
               _loc9_ = 0;
               while(_loc9_ < _loc8_)
               {
                  _loc10_ = DataGridBase(owner).mx_internal::visibleColumns[_loc9_];
                  _loc11_ = DataGridBase(owner).createColumnItemRenderer(_loc10_,false,_loc6_);
                  _loc12_ = new DataGridListData(_loc10_.itemToLabel(_loc6_),_loc10_.dataField,_loc10_.mx_internal::colNum,"",DataGridBase(owner));
                  _loc11_.styleName = DataGridBase(owner);
                  _loc5_.addChild(DisplayObject(_loc11_));
