package mx.controls.listClasses
{
   ...
   public class ListBase extends ScrollControlBase implements IDataRenderer, IFocusManagerComponent, IListItemRenderer, IDropInListItemRenderer, IEffectTargetHost
   {
      ...
      protected function mouseMoveHandler(param1:MouseEvent) : void
      {
         ...
         if(mx_internal::isPressed && mouseDownPoint && (Math.abs(mouseDownPoint.x - _loc2_.x) > mx_internal::DRAG_THRESHOLD || Math.abs(mouseDownPoint.y - _loc2_.y) > mx_internal::DRAG_THRESHOLD))
         {
         ...
         else if(!_loc3_ && Boolean(highlightItemRenderer))
         {
            if(!mx_internal::isPressed)
            {
               if(getStyle("useRollOver") && Boolean(highlightItemRenderer.data))
               {
                  mx_internal::clearHighlight(highlightItemRenderer);
               }
            }
      ...
      override public function set verticalScrollPosition(param1:Number) : void
      {
         ...
         if(_loc2_ != param1)
         {
            ...
            if((_loc5_ = Math.abs(_loc5_)) >= rowInfo.length - offscreenExtraRows || !iteratorValid)
            {
               clearIndicators();
               clearVisibleData();
      ...
      protected function dragScroll() : void
      {
         ...
         if(DragManager.isDragging && mx_internal::lastDragEvent && _loc3_ != verticalScrollPosition)
         {
      ...
      mx_internal function createItemMask(param1:Number, param2:Number, param3:Number, param4:Number, param5:DisplayObjectContainer = null) : DisplayObject
      {
         ...
         if(mx_internal::itemMaskFreeList.length > 0)
         {
      ...
      override protected function commitProperties() : void
      {
         ...
         if(columnCountChanged)
         {
            ...
            {
               mx_internal::setColumnWidth((explicitWidth - viewMetrics.left - viewMetrics.right) / columnCount);
            }
      ...
      mx_internal function addClipMask(param1:Boolean) : void
      {
         ...
         if(param1)
         {
            if(horizontalScrollBar && horizontalScrollBar.visible || mx_internal::hasOnlyTextRenderers() || runningDataEffect || listContent.bottomOffset != 0 || listContent.topOffset != 0 || listContent.leftOffset != 0 || listContent.rightOffset != 0)
            {
         ...
         var _loc2_:int = int(listItems.length - 1);
      ...
      protected function mouseDownHandler(param1:MouseEvent) : void
      {
         ...
         if(dragEnabled && Boolean(selectedData[rowMap[_loc2_.name].uid]))
         {
            mx_internal::bSelectOnRelease = true;
         }
      ...
      protected function drawItem(param1:IListItemRenderer, param2:Boolean = false, param3:Boolean = false, param4:Boolean = false, param5:Boolean = false) : void
      {
         ...
         var _loc8_:ListBaseContentHolder;
         if(!(_loc8_ = DisplayObject(param1).parent as ListBaseContentHolder))
         {
            return;
         }
         var _loc9_:Array = _loc8_.rowInfo;
         var _loc10_:Sprite = _loc8_.selectionLayer;
         var _loc11_:BaseListData;
         ...
         {
            if(Boolean(_loc11_) && Boolean(selectionIndicators[_loc11_.uid]))
            {
      ...
      override public function get baselinePosition() : Number
      {
         ...
         var _loc5_:ListBaseContentHolder;
      ...
      private function makeRowsAndColumnsWithExtraRows(param1:Number, param2:Number) : void
      {
         ...
         if(_loc9_ > 0 && !iterator.afterLast)
         {
            ...
            {
               if(_loc15_ = listItems.length - (_loc14_ + _loc7_.y))
               {
                  _loc16_ = 0;
         ...
         listContent.bottomOffset = offscreenExtraRowsBottom > 0 ? listItems[listItems.length - 1][0].y + rowHeight - _loc13_ + listContent.topOffset : 0;
         if(iteratorValid)
         {
      ...
      private function setSelectionDataLoop(param1:Array, param2:int, param3:Boolean = true) : void
      {
         ...
         else
         {
            ...
            while(Boolean(items.length) && !mx_internal::collectionIterator.afterLast)
            {
            ...
            len = int(selectionDataArray.length);
