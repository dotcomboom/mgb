   ...
   public class DividedBox extends Box
   {
      ...
      mx_internal function stopDividerDrag(param1:BoxDivider, param2:MouseEvent) : void
      {
         ...
         if(!liveDragging)
         {
            if(dragDelta == 0)
            {
               getDividerAt(activeDividerIndex).mx_internal::state = DividerState.OVER;
            }
            if(mx_internal::activeDivider)
            {
               dividerLayer.removeChild(mx_internal::activeDivider);
            }
            mx_internal::activeDivider = null;
            adjustChildSizes();
            invalidateSize();
            invalidateDisplayList();
         }
      ...
      private function postLayoutAdjustment() : void
      {
         ...
         var _loc1_:int = int(postLayoutChanges.length);
         var _loc2_:int = 0;
         while(_loc2_ < _loc1_)
         {
      ...
      public function get numDividers() : int
      {
         if(dividerLayer)
         {
            if(!liveDragging && Boolean(mx_internal::activeDivider))
            {
      ...
      mx_internal function changeCursor(param1:BoxDivider) : void
      {
         ...
         if(cursorID == CursorManager.NO_CURSOR)
         {
            _loc2_ = !!mx_internal::isVertical() ? getStyle("verticalDividerCursor") as Class : getStyle("horizontalDividerCursor") as Class;
      ...
      private function mouseMoveHandler(param1:MouseEvent) : void
      {
         ...
         {
            mx_internal::activeDivider.move(0,activeDividerStartPosition + dragDelta);
         }
         else
         {
            mx_internal::activeDivider.move(activeDividerStartPosition + dragDelta,0);
         }
      }
      ...
      mx_internal function startDividerDrag(param1:BoxDivider, param2:MouseEvent) : void
      {
         ...
         activeDividerIndex = mx_internal::getDividerIndex(param1);
         var _loc3_:DividerEvent = new DividerEvent(DividerEvent.DIVIDER_PRESS);
         _loc3_.dividerIndex = activeDividerIndex;
         dispatchEvent(_loc3_);
         if(liveDragging)
         {
            mx_internal::activeDivider = param1;
         }
         else
         {
            mx_internal::activeDivider = createDivider(PROXY_DIVIDER_INDEX);
            mx_internal::activeDivider.visible = false;
            mx_internal::activeDivider.mx_internal::state = DividerState.DOWN;
            mx_internal::activeDivider.setActualSize(param1.width,param1.height);
            mx_internal::activeDivider.move(param1.x,param1.y);
            mx_internal::activeDivider.visible = true;
            param1.mx_internal::state = DividerState.UP;
         }
         if(mx_internal::isVertical())
         {
            activeDividerStartPosition = mx_internal::activeDivider.y;
         }
         else
         {
            activeDividerStartPosition = mx_internal::activeDivider.x;
         }
      ...
      private function cacheSizes() : void
      {
         ...
         while(_loc4_ < _loc3_)
         {
            if((_loc5_ = IUIComponent(getChildAt(_loc4_))).includeInLayout)
            {
         ...
         oldChildSizes.push(new ChildSizeInfo(_loc2_ == Number.MAX_VALUE ? 1 : _loc2_));
      }
      ...
      private function getMousePosition(param1:MouseEvent) : Number
      {
         ...
         return !!mx_internal::isVertical() ? _loc2_.y : _loc2_.x;
      }
      ...
      override protected function measure() : void
      {
         ...
         var _loc1_:Boolean = Boolean(this.mx_internal::isVertical());
         var _loc2_:Number = 0;
         var _loc3_:Number = 0;
         var _loc4_:Number = 0;
         var _loc5_:Number = 0;
         ...
         while(_loc7_ < _loc6_)
         {
            if((_loc10_ = IUIComponent(getChildAt(_loc7_))).includeInLayout)
            {
               _loc11_ = Number(_loc10_.getExplicitOrMeasuredWidth());
               _loc12_ = Number(_loc10_.getExplicitOrMeasuredHeight());
               _loc13_ = Number(_loc10_.minWidth);
               _loc14_ = Number(_loc10_.minHeight);
               ...
               if(_loc1_)
               {
                  _loc2_ = Math.max(_loc15_ ? _loc13_ : _loc11_,_loc2_);
                  _loc4_ = Math.max(_loc11_,_loc4_);
                  _loc3_ += _loc16_ ? _loc18_ : _loc12_;
                  _loc5_ += _loc12_;
               }
               else
               {
                  _loc2_ += _loc15_ ? _loc17_ : _loc11_;
                  _loc4_ += _loc11_;
                  _loc3_ = Math.max(_loc16_ ? _loc14_ : _loc12_,_loc3_);
                  _loc5_ = Math.max(_loc12_,_loc5_);
               }
            }
            _loc7_++;
         }
         var _loc8_:Number = Number(mx_internal::layoutObject.mx_internal::widthPadding(numLayoutChildren));
         var _loc9_:Number = Number(mx_internal::layoutObject.mx_internal::heightPadding(numLayoutChildren));
      ...
      override protected function scrollChildren() : void
      {
         ...
         if(Boolean(mx_internal::contentPane) && Boolean(dividerLayer))
         {
            dividerLayer.scrollRect = mx_internal::contentPane.scrollRect;
         }
      }
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         if(!liveDragging && Boolean(mx_internal::activeDivider))
         {
            ...
            while(_loc4_ < _loc3_)
            {
               if((_loc8_ = IUIComponent(getChildAt(_loc4_))).includeInLayout)
               {
         ...
         while(_loc4_ < _loc3_)
         {
            if((_loc8_ = UIComponent(getChildAt(_loc4_))).includeInLayout)
            {
               if(_loc6_)
               {
      ...
      private function distributeDelta() : void
      {
         ...
         var _loc4_:Number;
         if((_loc4_ = oldChildSizes[_loc2_].size - Math.abs(dragDelta)) <= 0 || isNaN(_loc4_))
         {
            _loc4_ = 1;
         }
         var _loc11_:int = -1;
         ...
         while(_loc5_ >= 0)
         {
            ...
            _loc7_ = _loc14_ < 0 ? -Math.min(-_loc14_,_loc6_.deltaMin) : Math.min(_loc14_,_loc6_.deltaMax);
            _loc8_ = _loc6_.size + _loc7_;
            _loc14_ -= _loc7_;
            while(!(_loc9_ = IUIComponent(getChildAt(_loc13_--))).includeInLayout)
            {
            }
      ...
      private function preLayoutAdjustment() : void
      {
         ...
         while(_loc5_ < _loc4_)
         {
            if((_loc6_ = IUIComponent(getChildAt(_loc5_))).includeInLayout)
            {
         ...
         if(_loc2_ == 0 && _loc3_ == 0)
         {
            ...
            while(_loc5_ >= 0)
            {
               if((_loc6_ = UIComponent(getChildAt(_loc5_))).includeInLayout)
               {
                  _loc8_ = {"child":_loc6_};
                  if(_loc1_)
                  {
         ...
         else if(_loc2_ < 100)
         {
            ...
            while(_loc5_ < _loc4_)
            {
               if((_loc6_ = IUIComponent(getChildAt(_loc5_))).includeInLayout)
               {
                  ...
                  if(_loc1_)
                  {
                     _loc7_ = Number(_loc6_.percentHeight);
                     if(!isNaN(_loc7_))
                     {
                  ...
                  {
                     _loc7_ = Number(_loc6_.percentWidth);
                     if(!isNaN(_loc7_))
                     {
      ...
      private function computeAllowableMovement(param1:int) : void
      {
         ...
         while(_loc7_ >= 0)
         {
            ...
            _loc2_ += dontCoalesceDividers && Boolean(_loc2_) ? 0 : _loc8_.deltaMin;
            _loc3_ += dontCoalesceDividers && Boolean(_loc3_) ? 0 : _loc8_.deltaMax;
            _loc7_--;
         }
         _loc7_ = param1 + 1;
         while(_loc7_ < _loc6_)
         {
            _loc8_ = ChildSizeInfo(oldChildSizes[_loc7_]);
            _loc4_ += dontCoalesceDividers && Boolean(_loc4_) ? 0 : _loc8_.deltaMin;
            _loc5_ += dontCoalesceDividers && Boolean(_loc5_) ? 0 : _loc8_.deltaMax;
            _loc7_++;
         }
